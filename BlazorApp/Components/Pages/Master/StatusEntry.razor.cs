...
{
    public class StatusEntryBase : ComponentBase
    {
        ...
        [Parameter] public MStatus model { get; set; }
        ...
        protected async Task OnValidSubmit(EditContext context)
        {
            if (mode == "add")
            {
                var result = await masterHelper.InsertStatus(model);
                if (result.success)
                {
            ...
            {
                var result = await masterHelper.UpdateStatus(model);
                if (result.success)
                {
            ...
            {
                var result = await masterHelper.DeleteStatus(model);
                if (result.success)
                {
