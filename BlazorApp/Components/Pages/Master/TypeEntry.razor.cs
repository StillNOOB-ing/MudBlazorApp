...
{
    public class TypeEntryBase : ComponentBase
    {
        ...
        [Parameter] public MType model { get; set; }
        ...
        protected async Task OnValidSubmit(EditContext context)
        {
            if (mode == "add")
            {
                var result = await masterHelper.InsertType(model);
                if (result.success)
                {
            ...
            {
                var result = await masterHelper.UpdateType(model);
                if (result.success)
                {
            ...
            {
                var result = await masterHelper.DeleteType(model);
                if (result.success)
                {
